...
public class ST7735sDriver {
     ...
     * @throws IOException
     */
    public void drawImg16BitColorOptimization(BufferedImage bufferedImage) throws IOException {
        System.out.println("start to write Lcd Img ，drawImg16BitColorOptimization");
        BufferedImage highBitImage = new BufferedImage(ST7735sDriver.WIDTH, ST7735sDriver.HEIGHT, BufferedImage.TYPE_INT_RGB);
        BufferedImage lowBitImage = new BufferedImage(ST7735sDriver.WIDTH, ST7735sDriver.HEIGHT, BufferedImage.TYPE_INT_RGB);
        ...
        for (int y = 0; y < HEIGHT; y++) {
            for (int x = 0; x < WIDTH; x++) {
                ...
                highBitImage.setRGB(x,y,highBit);
                lowBitImage.setRGB(x,y,lowBit);
            }
        }

        for (int y = 0; y < HEIGHT; y++) {
            for (int x = 0; x < WIDTH; x++) {
                writeEveryPixel16Bit(highBitImage.getRGB(x,y), lowBitImage.getRGB(x,y));
            }
        }
    }
